...
const requestPayout = asyncHandler(async (req, res) => {
    ...
    if (payoutMode === 'automatic') {
        let currentCreator = await User.findById(creatorId);
        if (!currentCreator.paystackRecipientCode) {
            await payoutService.verifyBankAccount(currentCreator.payoutAccountNumber, currentCreator.payoutBankName);
            const recipientCode = await payoutService.createTransferRecipient(currentCreator);
            currentCreator.paystackRecipientCode = recipientCode;
            currentCreator = await currentCreator.save();
        }
        ...
        try {
            const transferResult = await payoutService.initiateTransfer(
                amountKobo,
                currentCreator.paystackRecipientCode,
                payout._id.toString()
            );
            payout.providerRef = transferResult.transfer_code;
            await payout.save();
