...
const getCreatorDetails = asyncHandler(async (req, res) => {
    ...
    // Run all data-gathering queries in parallel for efficiency
    const [financials, videos, payoutHistory] = await Promise.all([
        // 1. Get overall financial stats for the creator
        ...
        Video.aggregate([
            { $match: { creator: creatorId } },
            {
                $lookup: {
                    ...
                    let: { videoId: '$_id' },
                    pipeline: [
                        // Find successful transactions for the current video
                        { 
                            $match: {
                                $expr: {
                                    $and: [
                        ...
                        // For each transaction, look up the viewer's details
                        {
                            $lookup: {
                                from: 'users',
                                localField: 'viewer',
                        ...
                        // Deconstruct the viewerDetails array to get the object
                        { $unwind: '$viewerDetails' }
                    ],
                    as: 'sales'
                }
            },
            // Project the final shape of the data for each video
            {
                $project: {
                    title: 1,
                    priceKobo: 1,
        ...
        // 3. Get the creator's full payout history
        Payout.find({ creator: creatorId }).sort({ createdAt: -1 })
    ]);

    // Send the combined data as a single JSON response
    res.status(200).json({
        profile: creator,
