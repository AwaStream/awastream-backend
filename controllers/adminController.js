...
const getAdminDashboard = asyncHandler(async (req, res) => {
    ...
    // --- FIX 1: Include 'successful' for accurate total payouts ---
    const totalPayoutsAggregation = await Payout.aggregate([
    ...
    // --- FIX 2: Include 'pending' AND 'processing' for accurate pending list ---
    const pendingPayouts = await Payout.find({ status: { $in: ['pending', 'processing'] } })
        .populate('creator', 'userName email')
        .sort({ createdAt: -1 })
        .limit(10);
    // -----------------------------------------------------------------------
...
const getCreatorDetails = asyncHandler(async (req, res) => {
    ...
    const [financials, videos, payoutHistory] = await Promise.all([
        ...
        // --- FIX 3: Include 'successful' for the creator's payout history list ---
        Payout.find({ creator: creatorId }).sort({ createdAt: -1 })
        // ------------------------------------------------------------------------
    ]);
...
const getPayouts = asyncHandler(async (req, res) => {
    const { status } = req.query;
    // --- FIX 4: Allow filtering by 'successful' ---
    const query = status ? { status } : {};
    // ---------------------------------------------
...
const approvePayout = asyncHandler(async (req, res) => {
    ...
    // NOTE: If using automatic payouts (Nomba), 'approve' should trigger initiateTransfer
    // For manual approval, 'completed' is often used, but we should use 'successful' for consistency if possible.
    // Sticking to 'completed' here to minimize impact on existing manual flow, but 'successful' is preferred long-term.
    payout.status = 'completed'; 
    payout.processedBy = req.user.id;
    payout.processedAt = new Date();
    payout.notes = req.body.notes || 'Manually approved and paid.';
